...
impl QcpAdapter {
    ...
    async fn execute_qcp(&self, program: &str, context: serde_json::Value) -> Result<serde_json::Value> {
...
impl QcpAdapter {
    async fn parse_quantum_query(&self, _endpoint: &str, query: &str) -> Result<ContextNode> {
        ...
        let result = self.execute_qcp(Self::create_analysis_program(), context).await?;
                ...
                nodes.push(ContextNode {
                    ...
                    quantum_state: Some(QuantumState {
                        ...
                        phase: pattern.get("phase")
                            .and_then(|p| p.as_f64())
                            .unwrap_or(0.0),
                        collapse_probability: pattern.get("probability")
                            .and_then(|p| p.as_f64())
                            .unwrap_or(0.5),
                    }),
                    children: vec![],
