...
use super::{ContextContent, GatheredContext};
...
impl CollaborativeSessionTracker {
        ...
        match &context.content {
            ContextContent::Json(json) => {
                if let Some(messages) = json.get("messages").and_then(|m| m.as_array()) {
                    ...
                    let has_user = recent_messages
                        .iter()
                        .any(|m| m.get("role").and_then(|r| r.as_str()) == Some("user"));
                    let has_assistant = recent_messages
                        .iter()
    ...
    fn analyze_flow_state(&self, session: &CollaborativeSession) -> FlowState {
        let recent_interactions = session
            .interactions
            .iter()
            .rev()
            .take(10)
        ...
        if back_and_forth_count > 5 {
            ...
            let repeated: Vec<String> = repeated_concepts
                .into_iter()
    ...
    fn update_rapport_index(&mut self, session: &CollaborativeSession) {
        ...
        let session_score = (session.trust_flow_score.clarity
            ...
            + session.trust_flow_score.flow_depth)
            / 3.0;
        ...
        rapport.avg_session_productivity =
            rapport.avg_session_productivity * 0.9 + session_score * 0.1;
    }
    ...
    fn calculate_responsiveness(&self, session: &CollaborativeSession) -> f32 {
        ...
        for i in 1..session.interactions.len() {
            let time_diff =
                session.interactions[i].timestamp - session.interactions[i - 1].timestamp;
    ...
    fn detect_frustration(&self, session: &CollaborativeSession) -> f32 {
        ...
        for i in 1..session.interactions.len() {
            let gap = session.interactions[i].timestamp - session.interactions[i - 1].timestamp;
