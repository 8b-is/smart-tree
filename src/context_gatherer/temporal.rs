...
use super::{GatheredContext, ContextType};
...
impl TemporalContextAnalyzer {
        ...
        for (i, point) in timeline.iter().enumerate() {
            if let Some(ref mut session) = current_session {
                ...
                if gap > session_gap {
                    ...
                    session.average_intensity = 
                        (session.average_intensity + point.intensity) / 2.0;
                }
            } else {
                // Start first session
                current_session = Some(WorkSession {
    ...
    fn detect_periodic_patterns(&self, timeline: &[TimelinePoint]) -> Vec<PeriodicPattern> {
        ...
        if matches!(self.resolution, TemporalResolution::Day | TemporalResolution::Hour) {
    ...
    fn detect_daily_pattern(&self, timeline: &[TimelinePoint]) -> Option<PeriodicPattern> {
        ...
        for point in timeline {
            ...
            hour_activities.entry(hour).or_insert_with(Vec::new)
                .push(point.intensity);
        }
        
        // Find peak hours
        let mut peak_hours = hour_activities.iter()
    ...
    fn detect_weekly_pattern(&self, timeline: &[TimelinePoint]) -> Option<PeriodicPattern> {
        ...
        for point in timeline {
            ...
            day_activities.entry(day).or_insert_with(Vec::new)
                .push(point.intensity);
        }
        
        // Find peak days
        let mut peak_days = day_activities.iter()
            .map(|(day, intensities)| {
                let avg = intensities.iter().sum::<f32>() / intensities.len() as f32;
                (*day, avg)
            })
    ...
    fn calculate_total_duration(&self) -> Duration {
        ...
        let min_time = self.contexts.iter()
            .map(|c| c.timestamp)
            .min()
            .unwrap();
        let max_time = self.contexts.iter()
            .map(|c| c.timestamp)
            .max()
            .unwrap();
    ...
    fn count_active_days(&self) -> usize {
        let mut days = self.contexts.iter()
...
impl TemporalWaveGrid {
    ...
    pub fn add_wave(&mut self, wave: TemporalWave) {
        ...
        self.time_slots.entry(time_slot)
    ...
    pub fn get_interference_at(&self, time: DateTime<Utc>) -> f32 {
        ...
        if let Some(indices) = self.time_slots.get(&time_slot) {
            ...
            for &idx in indices {
                ...
                let value = wave.amplitude * age_factor * 
                    (2.0 * std::f32::consts::PI * wave.frequency * 
                     time.timestamp() as f32 + wave.phase).sin();
                total += value;
            }
    ...
    pub fn find_resonance_peaks(&self) -> Vec<DateTime<Utc>> {
        ...
        for (&time, indices) in &self.time_slots {
            if indices.len() > 3 { // Multiple waves at same time
