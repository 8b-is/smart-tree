...
pub const CONTEXT_EXTENSIONS: &[&str] = &[
    "json", "jsonl", "xml", "yaml", "yml", "toml", "md", 
    "txt", "log", "conf", "config", "env", "settings"
];
...
impl ContextGatherer {
    ...
    pub fn gather_all(&mut self) -> Result<()> {
        ...
        for custom_dir in custom_dirs {
            if custom_dir.exists() {
                ...
                let tool_name = custom_dir.file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("custom");
        ...
        let patterns = self.cross_session_bridge.analyze_for_patterns(&self.gathered_contexts);
    ...
    fn parse_jsonl(&self, content: &str, path: &Path) -> Result<(ContextType, ContextContent)> {
        ...
        Ok((content_type, ContextContent::Json(serde_json::Value::Array(lines))))
    }
    
    /// Parse XML content
    fn parse_xml(&self, content: &str) -> Result<(ContextType, ContextContent)> {
        // For now, store as text - could add proper XML parsing later
        Ok((ContextType::Configuration, ContextContent::Xml(content.to_string())))
    }
    ...
    fn is_relevant(&self, context: &GatheredContext) -> bool {
        ...
        if content_str.to_lowercase().contains(&project_name.to_lowercase()) {
            return true;
        }
    ...
    fn score_relevance(&mut self) {
        ...
        for context in &mut self.gathered_contexts {
            ...
            let mentions = content_str.to_lowercase()
        ...
        // Sort by relevance
        self.gathered_contexts.sort_by(|a, b| 
    ...
    fn extract_metadata(&self, path: &Path) -> HashMap<String, String> {
        ...
        if let Some(parent) = path.parent() {
            metadata.insert("parent_dir".to_string(), parent.to_string_lossy().to_string());
        }
        
        if let Ok(file_metadata) = fs::metadata(path) {
            if let Ok(modified) = file_metadata.modified() {
                metadata.insert("modified".to_string(), 
    ...
    pub fn analyze_temporal(&self, resolution: temporal::TemporalResolution) -> temporal::TemporalPatterns {
        let analyzer = temporal::TemporalContextAnalyzer::new(
    ...
    pub fn create_temporal_waves(&self, resolution: temporal::TemporalResolution) -> temporal::TemporalWaveGrid {
        let analyzer = temporal::TemporalContextAnalyzer::new(
            self.gathered_contexts.clone(),
            resolution
        );
    ...
    pub fn analyze_partnership(&self) -> partnership::PartnershipAnalysis {
        let analyzer = partnership::PartnershipAnalyzer::new(
            self.gathered_contexts.clone()
        );
    ...
    pub fn anchor_memory(
        ...
        self.session_tracker.anchor_memory(origin, anchor_type, context, keywords)
    }
    
    /// Find memories related to keywords
    pub fn find_relevant_memories(&self, keywords: &[String]) -> Vec<String> {
        self.session_tracker.find_relevant_anchors(keywords)
            .into_iter()
            .map(|anchor| format!(
                "[{}] {}: {} (keywords: {})",
    ...
    pub fn get_co_engagement_heatmap(&self) -> collab_session::CoEngagementHeatmap {
        let sessions: Vec<_> = self.session_tracker.session_history.iter().cloned().collect();
    ...
    pub fn get_relevant_insights(&self, keywords: &[String]) -> Vec<cross_session::CrossSessionInsight> {
        self.cross_session_bridge.suggest_relevant_insights(&self.project_path, keywords)
    }
    
    /// Invite a persona for consultation
    pub fn invite_persona(&self, context: &str, duration: u32) -> Option<cross_session::PersonaInvitation> {
