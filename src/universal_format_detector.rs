...
impl UniversalFormatDetector {
    ...
    pub fn detect_format(&mut self, content: &str) -> DataFormat {
        ...
        if lower_content.contains("<html") || lower_content.contains("<!doctype") ||
           lower_content.contains("<div") || lower_content.contains("<span") ||
           lower_content.contains("<p>") || lower_content.contains("<br") {
            self.format = DataFormat::HTML;
        } else if angle_brackets > total_chars / 20 {  // >5% angle brackets
            self.format = DataFormat::XML;
        } else if curly_braces > total_chars / 30 {  // >3.3% curly braces
            ...
            match self.format {
                DataFormat::HTML | DataFormat::XML => {
                    ...
                    for ch in line.chars() {
                        match ch {
                            '<' => {
                                if !line.contains("</") {  // Opening tag
                                    line_pattern.opener_count += 1;
                                }
                            },
                            '>' => {
                                line_pattern.closer_count += 1;
                                if line.contains("</") {  // Closing tag
                                    current_depth = current_depth.saturating_sub(1);
                                } else if !line.contains("/>") {  // Not self-closing
                                    current_depth += 1;
                                }
                            },
                            _ => {}
    ...
    fn detect_conversation_block(&mut self, block: &[String], start_line: usize) {
        ...
        let signature = format!(
            "d{}_s{}_l{}",
            self.pattern.line_patterns.last().map(|p| p.depth).unwrap_or(0),
            block_text.len(),
            block.len()
        );
        
        *self.participant_patterns.entry(signature.clone()).or_default() += 1;
        
        self.conversations.push(ConversationBlock {
            start_line,
            end_line: start_line + block.len(),
            depth: self.pattern.line_patterns.last().map(|p| p.depth).unwrap_or(0),
    ...
    pub fn tokenize_structure(&mut self) -> HashMap<String, u8> {
        ...
        let mut pattern_freq: Vec<(String, usize)> = 
            self.participant_patterns.iter()
                .map(|(k, v)| (k.clone(), *v))
                .collect();
    ...
    pub fn get_conversation_summary(&self) -> String {
        ...
        summary.push_str(&format!("Average spacing: {:.1}\n", self.pattern.average_spacing));
        ...
        for conv in &self.conversations {
            *participant_counts.entry(conv.participant.clone()).or_default() += 1;
        }
