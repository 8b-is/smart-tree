...
impl ProjectRenamer {
    ...
    fn generate_name_variants(old_name: &str, new_name: &str) -> HashMap<NamingConvention, (String, String)> {
        ...
        variants.insert(
            ...
            (Self::to_snake_case(&old_words), Self::to_snake_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::CamelCase,
            (Self::to_camel_case(&old_words), Self::to_camel_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::PascalCase,
            (Self::to_pascal_case(&old_words), Self::to_pascal_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::KebabCase,
            (Self::to_kebab_case(&old_words), Self::to_kebab_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::TitleCase,
            (Self::to_title_case(&old_words), Self::to_title_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::UpperCase,
            (Self::to_upper_case(&old_words), Self::to_upper_case(&new_words))
        );
        
        variants.insert(
            NamingConvention::LowerCase,
            (Self::to_lower_case(&old_words), Self::to_lower_case(&new_words))
        );
    ...
    fn to_title_case(words: &[String]) -> String {
        words.iter().map(|word| Self::capitalize(word)).collect::<Vec<_>>().join(" ")
    }
    ...
    pub async fn scan_project(&mut self, project_path: &Path) -> Result<()> {
        println!("🔎 Scanning for legacy references to \"{}\"...", self.config.old_name);
    ...
    pub async fn apply_renames(&self) -> Result<()> {
        ...
        for op in &self.operations {
            ops_by_file.entry(op.file_path.clone()).or_default().push(op);
        }
    ...
    async fn apply_file_renames(&self, file_path: &Path, operations: Vec<&RenameOperation>) -> Result<()> {
        ...
        if self.config.backup {
            let backup_path = file_path.with_extension(
                format!("{}.bak", file_path.extension().unwrap_or_default().to_str().unwrap_or(""))
            );
    ...
    pub fn show_summary(&self) {
        ...
        for op in &self.operations {
            let ext = op.file_path.extension()
                .and_then(|e| e.to_str())
                .unwrap_or("other");
        ...
        println!("   • Identifiers → `{}`", self.name_variants.get(&NamingConvention::SnakeCase).unwrap().1);
        println!("   • Strings → \"{}\"", self.config.new_name);
        println!("   • Titles → `{}`", self.name_variants.get(&NamingConvention::TitleCase).unwrap().1);
...
impl ProjectRenamer {
    ...
    pub fn show_preview(&self) {
        for (i, op) in self.operations.iter().take(10).enumerate() {
            ...
            println!("   Context: {} (confidence: {:.0}%)", op.context, op.confidence * 100.0);
        }
