...
impl DynamicTokenizer {
    ...
    pub fn analyze(&mut self, nodes: &[FileNode]) {
        for node in nodes {
            ...
            for component in path_str.split('/').filter(|c| !c.is_empty()) {
                *self.path_components.entry(component.to_string()).or_insert(0) += 1;
            }
    ...
    fn analyze_name_patterns(&mut self, name: &str) {
        ...
        // Common suffixes
        let suffix_patterns = ["_test", "Test", "Spec", "_spec", ".test", ".spec", 
                               "Controller", "Service", "Repository", "Model", 
                               "View", "Component", "Module", "Config"];
    ...
    fn generate_tokens(&mut self) {
        ...
        for (pattern, count) in &self.path_components {
            if *count > 2 { // Only tokenize if it appears more than twice
        ...
        for (pattern, count) in &self.extensions {
            if *count > 5 { // Extensions need higher frequency
        ...
        for (pattern, _count) in all_patterns.iter().take(127) { // Max 127 tokens
            self.tokens.insert(pattern.clone(), format!("{:02X}", token_id));
            token_id += 1;
            if token_id > 0xFE { // Reserve 0xFF
                break;
            }
        }
    }
    ...
    pub fn get_stats(&self) -> TokenizerStats {
        ...
        TokenizerStats {
            patterns_found: self.path_components.len() + self.file_names.len() + self.extensions.len(),
...
mod tests {
    ...
    fn test_camel_case_split() {
        assert_eq!(split_camel_case("UserController"), vec!["user", "controller"]);
        assert_eq!(split_camel_case("HTTPSConnection"), vec!["h", "t", "t", "p", "s", "connection"]);
    }
    ...
    fn test_pattern_detection() {
        ...
        for pattern in patterns {
            ...
            for component in pattern.split('/') {
                *tokenizer.path_components.entry(component.to_string()).or_insert(0) += 1;
            }
        }
    ...
    fn test_compression() {
        ...
        // Add patterns
        for _ in 0..10 {
            *tokenizer.path_components.entry("src".to_string()).or_insert(0) += 1;
            *tokenizer.path_components.entry("components".to_string()).or_insert(0) += 1;
        }
    ...
    fn test_token_assignment_order() {
        ...
        *tokenizer.path_components.entry("very_frequent".to_string()).or_insert(0) = 100;
        *tokenizer.path_components.entry("less_frequent".to_string()).or_insert(0) = 50;
        *tokenizer.path_components.entry("rare".to_string()).or_insert(0) = 3;
        *tokenizer.path_components.entry("too_rare".to_string()).or_insert(0) = 1; // Won't be tokenized
