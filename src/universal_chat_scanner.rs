...
impl UniversalChatScanner {
    ...
    async fn scan_cursor_windsurf(&mut self) -> Result<()> {
        ...
        if Path::new(windsurf_path.as_ref()).exists() {
            ...
            count += self.scan_directory(windsurf_path.as_ref(), "windsurf").await?;
        }
            ...
            if let Ok(file_path) = entry {
                if let Ok(content) = fs::read_to_string(&file_path) {
                    ...
                    if self.looks_like_chat(&content) {
                        let chat = self.create_chat_from_content(
                            &content,
                            source,
                            &file_path
                        )?;
    ...
    fn parse_claude_chat(&self, content: &str, path: &Path) -> Result<UniversalChat> {
        ...
        Ok(UniversalChat {
            source: ChatSource::Claude { project: project.clone() },
    ...
    fn extract_keywords(&self, content: &str) -> Vec<String> {
        ...
        let tech_words = ["function", "async", "memory", "audio", "tokenization",
    ...
    fn calculate_importance(&self, content: &str) -> f32 {
        ...
        if content.contains("fix") || content.contains("solve") || 
           content.contains("implement") || content.contains("breakthrough") {
            score += 0.2;
        }
    ...
    pub async fn save_to_m8(
        &self,
        destination: &MemoryDestination
    ) -> Result<()> {
        let base_path = match destination.memory_type {
            MemoryType::ProjectMemory => {
                format!("~/.mem8/projects/{}", 
                    destination.project.as_ref().unwrap_or(&"default".to_string()))
            },
