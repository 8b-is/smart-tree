...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Compress { input, output, zlib, semantic } => {
        ...
        Commands::Stats { input } => {
            ...
            println!("Reduction factor: {:>8.1}x", original_size as f64 / compressed_size as f64);
        ...
        Commands::Inspect { input } => {
            ...
            for line in &lines {
                if line.contains("::section:") && line.ends_with("::") {
                    if let Some(section) = line.strip_prefix("::section:").and_then(|s| s.strip_suffix("::")) {
            ...
            if !most_frequent_token.is_empty() {
                println!("🧠 High-frequency token: {} (\"{}\")", most_frequent_token, most_frequent_pattern);
            }
        ...
        Commands::Aggregate { path, output, max_depth, pattern: _, exclude, zlib, semantic } => {
            ...
            println!("📚 Found {} markdown files to aggregate", markdown_files.len());
            ...
            let size_actual = format!("{} {} {}", timestamp, combined_for_analysis.len(), compressed_size);
            ...
            // Print summary
            let compression_ratio = (total_original_size as f64 - compressed_size as f64) / total_original_size as f64 * 100.0;
