...
impl M8Header {
    ...
    fn validate(&self) -> Result<()> {
        if &self.magic != MAGIC {
            anyhow::bail!(
                "Invalid magic: expected MEM8, got {:?}",
        ...
        if self.version != VERSION {
            eprintln!(
                "Warning: Version mismatch. Expected {}, got {}",
...
fn validate_file(path: &PathBuf, verbose: bool) -> Result<()> {
    let mut file = File::open(path).context("Failed to open MEM8 file")?;
...
fn inspect_file(path: &PathBuf, hex: bool, limit: Option<usize>) -> Result<()> {
    ...
    if limit.is_some() && max_sections < header.section_count as usize {
        println!(
            "... {} more sections",
            header.section_count as usize - max_sections
        );
    }
