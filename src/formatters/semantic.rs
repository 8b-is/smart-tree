...
impl Formatter for SemanticFormatter {
    fn format(
        ...
        writeln!(writer, "{}", "Grouping files by conceptual similarity...".dimmed())?;
        ...
        for node in nodes {
            ...
            groups.entry(category).or_insert_with(Vec::new).push(node.clone());
        }
            ...
            if let Some(files) = groups.get(&category) {
                ...
                for (idx, node) in sorted_files.iter().enumerate() {
                    ...
                    let size_str = if node.is_dir {
                        ...
                        format!("({})", Self::format_size(node.size)).dimmed().to_string()
                    };
        ...
        writeln!(writer, "Total files: {} | Total directories: {} | Total size: {}",
        ...
        writeln!(writer, "Semantic diversity: {} categories ({:.0}% coverage)",
        ...
        writeln!(writer)?;
        writeln!(writer, "{}", "✨ \"Every file carries waves of meaning\" - Omni ✨".italic().dimmed())?;
