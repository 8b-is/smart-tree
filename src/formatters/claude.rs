...
impl ClaudeFormatter {
    ...
    fn write_api_wrapper(
        ...
        if self.include_context {
            ...
            writeln!(
                writer,
                r#"    "description": "Ultra-compressed directory structure using Smart Tree quantum format","#
            )?;
            writeln!(writer, r#"    "features": ["#)?;
            writeln!(
                writer,
                r#"      "Bitfield headers for efficient metadata encoding","#
            )?;
            writeln!(
                writer,
                r#"      "Token substitution (e.g., 0x80=node_modules, 0x91=.rs)","#
            )?;
            ...
            writeln!(
                writer,
                r#"      "tokens_saved": {}"#,
                original_estimate.saturating_sub(binary.len() as u64)
            )?;
            writeln!(writer, r#"    }}"#)?;
            writeln!(writer, r#"  }},"#)?;
        }
        ...
        writeln!(
            writer,
            r#"    "The data_base64 field contains the binary quantum format encoded in base64","#
        )?;
        writeln!(
            writer,
            r#"    "Use the header information to understand token mappings","#
        )?;
        writeln!(
            writer,
            r#"    "ASCII codes: 0x0E=enter dir, 0x0F=exit dir, 0x0B=same level","#
        )?;
        writeln!(
            writer,
            r#"    "Permission deltas are XOR differences from parent (e.g., 0x0049 = 755^644)""#
        )?;
        writeln!(writer, r#"  ]"#)?;
...
impl Formatter for ClaudeFormatter {
    fn format(
        ...
        self.quantum_formatter
            .format(&mut quantum_buffer, nodes, stats, root_path)?;
