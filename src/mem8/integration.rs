...
use crate::mem8::{
    wave::{MemoryWave, WaveGrid, FrequencyBand},
    reactive::{ReactiveMemory, ReactiveLayer, ReactivePattern, ReactiveResponse, SensorInput},
    consciousness::ConsciousnessEngine,
    format::{M8Writer, CompressedWave},
};
...
impl SmartTreeMem8 {
    ...
    fn create_directory_wave(&self, metadata: &DirectoryMetadata) -> MemoryWave {
        ...
        let amplitude = (metadata.importance * 0.7 + metadata.normalized_size * 0.3).clamp(0.1, 1.0);
    ...
    pub fn process_directory_event(&self, event: DirectoryEvent) -> Option<ReactiveResponse> {
        let input = match event {
            DirectoryEvent::LargeDirectory { size, .. } => {
                SensorInput::Visual {
            ...
            DirectoryEvent::SecurityThreat { severity, .. } => {
                SensorInput::Threat {
                    severity,
                    proximity: 1.0,
            ...
            DirectoryEvent::RapidChange { rate, .. } => {
                SensorInput::Visual {
                    intensity: 0.5,
                    motion: rate,
                    looming: false,
    ...
    pub fn export_memories<W: std::io::Write>(&self, writer: W) -> Result<()> {
        ...
        for x in 0..=255u8 {
            for y in 0..=255u8 {
                for z in 0..100u16 { // Sample first 100 layers
                    if let Some(wave) = grid.get(x, y, z) {
...
pub fn integrate_with_smart_tree() -> Result<()> {
    ...
    if let Some(response) = mem8.process_directory_event(event) {
        println!("Reactive response: {} ({}ms)", response.action, response.latency.as_millis());
    }
