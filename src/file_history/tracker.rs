    ...
    pub fn track_write(
        ...
        let bytes_affected = match operation {
            FileOperation::Append => {
                new_content.len() - old_content.map(|s| s.len()).unwrap_or(0)
            }
            ...
            if entry.path().extension().and_then(|s| s.to_str()) == Some("flg") {
                ...
                for line in contents.lines() {
                    if let Ok(log_entry) = serde_json::from_str::<LogEntry>(line) {
                        ...
                        file_ops.entry(log_entry.file_path.clone())
                            .or_insert_with(Vec::new)
                            .push(log_entry.operation);
                        
                        summary.operation_counts.entry(log_entry.operation)
                            .and_modify(|c| *c += 1)
                            .or_insert(1);
                    }
                }
...
mod tests {
    ...
    fn test_tracker_basic() -> Result<()> {
        ...
        // Append and track
        let op = tracker.track_write(&test_file, Some("hello"), "hello world", "test-agent", "session-1")?;
