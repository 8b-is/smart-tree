...
impl UnifiedSearch {
    ...
    pub fn search(
        ...
        let results = match parsed_query.intent {
            ...
            SearchIntent::FindSecurity => self.search_security(path, &parsed_query, &task_context)?,
            SearchIntent::FindPerformance => self.search_performance(path, &parsed_query, &task_context)?,
    ...
    /// Create task context from parsed query
    fn create_task_context(&self, parsed_query: &ParsedQuery, max_results: Option<usize>) -> TaskContext {
        TaskContext {
            task: parsed_query.original_query.clone(),
            focus_areas: parsed_query.focus_areas.clone(),
            relevance_threshold: if parsed_query.confidence > 0.8 { 0.7 } else { 0.5 },
            max_results,
        }
    }
            ...
            if self.is_code_file(&node) {
                ...
                if relevance.score >= context.relevance_threshold {
                    let result = SearchResult {
                        ...
                        snippet: None,
                        line_number: None,
                        suggested_actions: vec!["Smart read".to_string(), "Analyze code".to_string()],
                    };
                    results.push(result);
                }
            }
            ...
            if self.is_config_file(&node) {
                ...
                if relevance.score >= context.relevance_threshold {
                    let result = SearchResult {
                        ...
                        snippet: None,
                        line_number: None,
                        suggested_actions: vec!["Edit config".to_string(), "Validate syntax".to_string()],
                    };
                    results.push(result);
                }
            }
            ...
            if self.is_test_file(&node) {
                ...
                if relevance.score >= context.relevance_threshold {
                    let result = SearchResult {
                        ...
                        snippet: None,
                        line_number: None,
                        suggested_actions: vec!["Run tests".to_string(), "Analyze coverage".to_string()],
                    };
                    results.push(result);
                }
            }
    ...
    fn generate_search_suggestions(
        ...
        if primary.is_empty() && secondary.is_empty() {
            ...
            suggestions.push("No high-priority results. Consider lowering relevance threshold.".to_string());
        }
        ...
        match parsed_query.intent {
            ...
            SearchIntent::FindConfig => {
                suggestions.push("Use find_config_files for comprehensive configuration discovery.".to_string());
            }
            SearchIntent::FindTests => {
                suggestions.push("Use find_tests to discover all test files and patterns.".to_string());
            }
            _ => {}
        }
