...
impl ContextAnalyzer {
    ...
    fn initialize_focus_keywords(&mut self) {
        ...
        self.focus_keywords.insert(
            ...
            vec![
                ...
                "signin", "signup", "credential", "authenticate", "authorize"
        ...
        self.focus_keywords.insert(
            ...
            vec![
                ...
                "rest", "graphql", "controller", "service", "client"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            FocusArea::Database,
            vec![
                "db", "database", "sql", "query", "table", "schema", "migration",
                "model", "entity", "repository", "dao", "orm"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            FocusArea::Frontend,
            vec![
                "ui", "component", "react", "vue", "angular", "html", "css", "js",
                "frontend", "client", "view", "template", "style"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            FocusArea::Backend,
            vec![
                "server", "service", "controller", "model", "business", "logic",
                "backend", "core", "engine", "processor"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            FocusArea::Testing,
            vec![
                "test", "spec", "mock", "assert", "expect", "unit", "integration",
                "e2e", "fixture", "stub", "spy"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            ...
            vec![
                "config", "env", "settings", "properties", "yaml", "json", "toml",
                "ini", "conf", "cfg", "setup"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            FocusArea::Security,
            vec![
                "security", "vulnerability", "sanitize", "validate", "encrypt", "hash",
                "secure", "crypto", "ssl", "tls", "cert"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            ...
            vec![
                "performance", "optimize", "cache", "memory", "cpu", "benchmark",
                "perf", "speed", "fast", "efficient"
            ].into_iter().map(String::from).collect(),
        );
        ...
        self.focus_keywords.insert(
            ...
            vec![
                "doc", "readme", "comment", "documentation", "guide", "manual",
                "help", "tutorial", "example"
            ].into_iter().map(String::from).collect(),
        );
    }
    ...
    fn is_important_file(&self, filename: &str) -> bool {
        matches!(filename,
            "readme.md" | "cargo.toml" | "package.json" | "requirements.txt" |
            "dockerfile" | "docker-compose.yml" | "makefile" | ".gitignore" |
            "main.rs" | "lib.rs" | "mod.rs" | "index.js" | "app.py" | "main.py"
        )
    }
    ...
    fn is_important_directory(&self, dirname: &str) -> bool {
        matches!(dirname,
            "src" | "lib" | "api" | "server" | "client" | "frontend" | "backend" |
            "components" | "services" | "controllers" | "models" | "routes" |
            "config" | "configs" | "auth" | "authentication"
        )
    }
    ...
    fn is_unimportant_directory(&self, dirname: &str) -> bool {
        matches!(dirname,
            "node_modules" | "target" | "dist" | "build" | ".git" | ".vscode" |
            "vendor" | "__pycache__" | ".pytest_cache" | "coverage" | "logs"
        )
    }
}
