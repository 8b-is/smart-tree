...
pub fn get_file_emoji(node: &FileNode, no_emoji: bool) -> &'static str {
    ...
    if node.is_dir {
        return if no_emoji {
            if node.size == 0 { "[EMPTY_DIR]" } else { "[DIR]" }
        } else {
            if node.size == 0 { "📂" } else { "📁" }
        };
    }
    ...
    match node.category {
        ...
        FileCategory::Rust => if no_emoji { "[RUST]" } else { "🦀" },
        FileCategory::Python => if no_emoji { "[PY]" } else { "🐍" },
        FileCategory::JavaScript => if no_emoji { "[JS]" } else { "📜" },
        FileCategory::TypeScript => if no_emoji { "[TS]" } else { "📘" },
        FileCategory::Java => if no_emoji { "[JAVA]" } else { "☕" },
        FileCategory::C => if no_emoji { "[C]" } else { "🔷" },
        FileCategory::Cpp => if no_emoji { "[CPP]" } else { "🔷" },
        FileCategory::Go => if no_emoji { "[GO]" } else { "🐹" },
        FileCategory::Ruby => if no_emoji { "[RB]" } else { "💎" },
        FileCategory::PHP => if no_emoji { "[PHP]" } else { "🐘" },
        FileCategory::Shell => if no_emoji { "[SH]" } else { "🐚" },
        
        // Markup & Data
        FileCategory::Markdown => if no_emoji { "[MD]" } else { "📝" },
        FileCategory::Html => if no_emoji { "[HTML]" } else { "🌐" },
        FileCategory::Css => if no_emoji { "[CSS]" } else { "🎨" },
        FileCategory::Json => if no_emoji { "[JSON]" } else { "📊" },
        FileCategory::Yaml => if no_emoji { "[YAML]" } else { "📋" },
        FileCategory::Xml => if no_emoji { "[XML]" } else { "📰" },
        FileCategory::Toml => if no_emoji { "[TOML]" } else { "🔧" },
        FileCategory::Csv => if no_emoji { "[CSV]" } else { "📊" },
        
        // Build & Config
        FileCategory::Makefile => if no_emoji { "[MAKE]" } else { "🔨" },
        FileCategory::Dockerfile => if no_emoji { "[DOCKER]" } else { "🐳" },
        FileCategory::GitConfig => if no_emoji { "[GIT]" } else { "📝" },
        FileCategory::Config => if no_emoji { "[CFG]" } else { "⚙️" },
        
        // Archives
        FileCategory::Archive => if no_emoji { "[ZIP]" } else { "📦" },
        
        // Media
        FileCategory::Image => if no_emoji { "[IMG]" } else { "🖼️ " },
        FileCategory::Video => if no_emoji { "[VID]" } else { "🎬" },
        FileCategory::Audio => if no_emoji { "[AUD]" } else { "🎵" },
        
        // Office & Documents
        FileCategory::Office => if no_emoji { "[DOC]" } else { "📄" },
        FileCategory::Spreadsheet => if no_emoji { "[XLS]" } else { "📊" },
        FileCategory::PowerPoint => if no_emoji { "[PPT]" } else { "📊" },
        FileCategory::Pdf => if no_emoji { "[PDF]" } else { "📕" },
        FileCategory::Ebook => if no_emoji { "[BOOK]" } else { "📚" },
        
        // Text Variants
        FileCategory::Txt => if no_emoji { "[TXT]" } else { "📄" },
        FileCategory::Rtf => if no_emoji { "[RTF]" } else { "📄" },
        FileCategory::Log => if no_emoji { "[LOG]" } else { "📋" },
        FileCategory::License => if no_emoji { "[LIC]" } else { "📜" },
        FileCategory::Readme => if no_emoji { "[README]" } else { "📖" },
        
        // Security & Crypto
        FileCategory::Certificate => if no_emoji { "[CERT]" } else { "🔐" },
        FileCategory::Encrypted => if no_emoji { "[ENC]" } else { "🔒" },
        
        // System & Binary
        FileCategory::SystemFile => if no_emoji { "[SYS]" } else { "⚙️ " },
        FileCategory::Binary => if no_emoji { "[BIN]" } else { "⚙️ " },
        
        // Database
        FileCategory::Database => if no_emoji { "[DB]" } else { "🗄️ " },
        
        // Fonts
        FileCategory::Font => if no_emoji { "[FONT]" } else { "🔤" },
        
        // Disk Images
        FileCategory::DiskImage => if no_emoji { "[DISK]" } else { "💿" },
        
        // 3D & CAD
        FileCategory::Model3D => if no_emoji { "[3D]" } else { "🎲" },
        
        // Scientific & Data
        FileCategory::Jupyter => if no_emoji { "[JUPYTER]" } else { "📓" },
        FileCategory::RData => if no_emoji { "[RDATA]" } else { "📊" },
        FileCategory::Matlab => if no_emoji { "[MAT]" } else { "📐" },
        
        // Web Assets
        FileCategory::WebAsset => if no_emoji { "[WEB]" } else { "🌐" },
        
        // Package & Dependencies
        FileCategory::Package => if no_emoji { "[PKG]" } else { "📦" },
        FileCategory::Lock => if no_emoji { "[LOCK]" } else { "🔒" },
        
        // Testing
        FileCategory::Test => if no_emoji { "[TEST]" } else { "🧪" },
        
        // Memory Files (MEM|8!)
        FileCategory::Memory => if no_emoji { "[MEM8]" } else { "🧠" },
        
        // Others
        FileCategory::Backup => if no_emoji { "[BAK]" } else { "💾" },
        FileCategory::Temp => if no_emoji { "[TMP]" } else { "🗑️" },
        FileCategory::Unknown => {
            // Fall back to file type for unknowns
            match node.file_type {
                FileType::Symlink => if no_emoji { "[LINK]" } else { "🔗" },
                FileType::Executable => if no_emoji { "[EXE]" } else { "⚙️ " },
                FileType::Socket => if no_emoji { "[SOCK]" } else { "🔌" },
                FileType::Pipe => if no_emoji { "[PIPE]" } else { "🪄" },
                FileType::BlockDevice => if no_emoji { "[BLK]" } else { "💾" },
                FileType::CharDevice => if no_emoji { "[CHR]" } else { "📺" },
                FileType::RegularFile => {
                    if node.size == 0 {
                        if no_emoji { "[EMPTY]" } else { "🪹" }
                    } else {
                        if no_emoji { "[FILE]" } else { "📄" }
                    }
                }
                FileType::Directory => if no_emoji { "[DIR]" } else { "📁" },
            }
        }
    }
}
