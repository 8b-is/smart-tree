...
impl SmartTreeTerminal {
    ...
    fn draw_header(f: &mut Frame, area: Rect, _state: &TerminalState) {
        let header = Paragraph::new(Text::from(vec![
            Line::from(vec![
                Span::styled("Smart Tree Terminal", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
                Span::raw(" v4.0 - "),
    ...
    fn draw_history(f: &mut Frame, area: Rect, state: &TerminalState) {
        let history_items: Vec<ListItem> = state.command_history
            .iter()
            .rev()
    ...
    fn draw_suggestions(f: &mut Frame, area: Rect, state: &TerminalState) {
        let suggestion_items: Vec<ListItem> = state.suggestions
            .iter()
            .map(|s| {
                ListItem::new(vec![
                    Line::from(vec![
                    ...
                    Line::from(Span::styled(&s.description, Style::default().fg(Color::Gray))),
                ])
            })
            .collect();
    ...
    fn draw_input(f: &mut Frame, area: Rect, state: &TerminalState) {
        ...
        // Set cursor position
        f.set_cursor(
            area.x + state.cursor_pos as u16 + 1,
            area.y + 1,
        );
    }
