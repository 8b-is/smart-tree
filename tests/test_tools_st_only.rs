...
fn create_complex_test_directory() -> Result<TempDir> {
    ...
    fs::write(temp_dir.path().join("src/modules/core/engine.rs"), "pub struct Engine {\n    // TODO: Add fields\n}")?;
...
fn test_config_default() {
    ...
    assert!(!config.compress, "Compression should be disabled by default");
    assert!(config.st_binary.to_string_lossy().contains("st"), 
...
fn test_list_basic() -> Result<()> {
    ...
    assert!(result.contains("README.md") || result.contains("readme"), 
            "Should list README.md");
    assert!(result.contains("src") || result.contains("SRC"), 
...
fn test_list_with_file_type() -> Result<()> {
    ...
    assert!(result.contains(".rs") || result.contains("main") || result.contains("lib"), 
...
fn test_search_basic() -> Result<()> {
    ...
    assert!(result.contains("TODO") || result.contains("main.rs") || result.contains("engine"), 
...
fn test_search_with_file_type() -> Result<()> {
    ...
    assert!(result.contains("TODO") || result.contains("api") || result.contains("Document"), 
...
fn test_overview() -> Result<()> {
    ...
    assert!(result.contains("STATS") || result.contains("F") || result.contains("D"), 
...
fn test_overview_with_depth() -> Result<()> {
    ...
    for depth in depths {
        ...
        assert!(!result.is_empty(), "Overview with depth {} should produce output", depth);
    }
...
fn test_stats() -> Result<()> {
    ...
    assert!(result.contains("Files") || result.contains("files") || result.contains("F:"), 
            "Stats should show file count");
    assert!(result.contains("Directories") || result.contains("directories") || result.contains("D:"), 
...
fn test_semantic() -> Result<()> {
    ...
    assert!(!result.is_empty(), "Semantic analysis should produce output");
...
fn test_empty_directory_handling() -> Result<()> {
    ...
    let result = tools.stats(temp_dir.path())?;
    assert!(result.contains("0") || result.contains("empty"), 
...
fn test_nested_directory_operations() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        fs::create_dir(&path)?;
        fs::write(path.join(format!("file{}.txt", i)), format!("Content at level {}", i))?;
    }
    
    let tools = StOnlyTools::new();
    let result = tools.overview(temp_dir.path(), Some(15))?;
    
    assert!(result.contains("level9") || result.contains("9") || result.contains("file"), 
...
fn test_all_list_options_combined() -> Result<()> {
    ...
    assert!(!result.is_empty(), "Combined options should still produce output");
...
fn test_special_characters_in_filenames() -> Result<()> {
    ...
    assert!(!result.is_empty(), "Should list files with special characters");
...
fn test_large_file_handling() -> Result<()> {
    ...
    assert!(result.contains("1") || result.contains("MB") || result.contains("MiB"), 
