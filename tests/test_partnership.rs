...
use st::context_gatherer::{
    ...
    ContextContent, ContextGatherer, ContextType, GatherConfig, GatheredContext,
};
...
fn test_flow_state_detection() {
    ...
    match &session.flow_state {
        st::context_gatherer::collab_session::FlowState::Flow {
            depth,
            sustained_minutes,
        } => {
            assert!(*depth > 0.5);
...
fn test_partnership_analysis() {
    ...
    assert!(["Thriving", "Healthy", "Developing"]
        .contains(&analysis.partnership_health.status.as_str()));
...
fn test_co_engagement_heatmap() {
    ...
    let has_morning_peak = heatmap
        .peak_collaboration_zones
        .iter()
        .any(|(hour, _)| *hour == 9);
    let has_afternoon_peak = heatmap
        .peak_collaboration_zones
        .iter()
        .any(|(hour, _)| *hour == 14);
...
fn test_persona_invitation() {
    ...
    let invitation = bridge
        .invite_persona("Need to optimize the wave calculation performance", 15)
        .unwrap();

    assert_eq!(invitation.persona_name, "The Cheet");
    assert!(invitation
        .expertise_areas
        .contains(&"Performance optimization".to_string()));
    assert_eq!(invitation.suggested_duration_minutes, 15);

    // Test wave-related context
    let invitation = bridge
        .invite_persona("Exploring wave interference patterns in memory", 10)
        .unwrap();

    assert_eq!(invitation.persona_name, "Omni");
    assert!(invitation
        .expertise_areas
        .contains(&"Wave-based thinking".to_string()));
}
...
fn test_insight_generation() {
    ...
    let contexts: Vec<_> = (0..5)
        .map(|i| GatheredContext {
            source_path: PathBuf::from(format!("/project{}/code.rs", i)),
            ai_tool: "claude".to_string(),
            content_type: ContextType::CodeSnippets,
            content: ContextContent::Text("Using observer pattern for event handling".to_string()),
            metadata: HashMap::new(),
            relevance_score: 0.8,
