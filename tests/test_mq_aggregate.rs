...
fn test_mq_aggregate_basic() {
    ...
    fs::write(temp_path.join("INSTALL.md"), "# Installation\n\nRun `cargo install`.").unwrap();
    fs::create_dir(temp_path.join("docs")).unwrap();
    fs::write(temp_path.join("docs/API.md"), "# API Reference\n\n## Functions\n\nAPI docs here.").unwrap();
    ...
    assert!(output.status.success(), "mq aggregate failed: {:?}", String::from_utf8_lossy(&output.stderr));
...
fn test_mq_aggregate_with_compression() {
    ...
    assert!(aggregate_size < original_size, 
