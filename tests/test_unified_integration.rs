...
use st::tools_st_only::{StOnlyTools, StToolsConfig, ListOptions, SearchOptions};
use st::st_context_aware::{StContextTracker, ContextualOperation, WorkContext};
use anyhow::Result;
...
"#)?;
    ...
    fs::write(temp_dir.path().join("src/core/engine/mod.rs"), r#"
pub struct Engine {
    running: bool,
}
...
"#)?;
    ...
    fs::write(temp_dir.path().join("src/utils/helpers/mod.rs"), r#"
/// Helper utilities
...
"#)?;
    ...
    fs::write(temp_dir.path().join("tests/integration/test_api.rs"), r#"
#[test]
...
fn test_unified_tools_exploration_workflow() -> Result<()> {
    ...
    assert!(overview.contains("STATS"), "Quick overview should show stats");
    ...
    assert!(main_content.contains("fn main()"), "Should read main function");
    ...
    assert!(todos.contains("TODO") || todos.contains("main.rs"), "Should find TODO comments");
...
fn test_context_aware_development_workflow() -> Result<()> {
    ...
    assert!(suggestions.iter().any(|s| s.contains("test") || s.contains("relations")), 
...
fn test_debugging_workflow_with_all_tools() -> Result<()> {
    ...
    assert!(stats.contains("Files") || stats.contains("F:"), "Should show file stats");
    
    // Step 4: Context should recognize debugging pattern
    let context = tracker.analyze_context()?;
    assert!(matches!(context, WorkContext::Hunting { .. }), 
            "Multiple searches should trigger hunting context");
    
    // Step 5: Get debugging-specific suggestions
    let suggestions = tracker.get_suggestions(project.path());
    assert!(suggestions.iter().any(|s| s.contains("search") || s.contains("recent")), 
...
fn test_full_project_understanding_workflow() -> Result<()> {
    ...
    assert!(project_understanding.contains("QUICK OVERVIEW"), "Should have overview");
    assert!(project_understanding.contains("SEMANTIC GROUPS"), "Should have semantic analysis");
    assert!(project_understanding.contains("STATISTICS"), "Should have stats");
    ...
    assert!(!impl_blocks.is_empty() || impl_blocks.contains("impl"), 
    ...
    assert!(project.path().join(".st_context.json").exists(), 
...
fn test_cross_tool_consistency() -> Result<()> {
    ...
    for item in &["src", "tests", "Cargo.toml", "README.md"] {
        assert!(unified_ls.contains(item) || tools_ls.contains(item), 
                "Both tools should find {}", item);
    }
    
    // Compare search results
    let unified_search = unified.grep("TODO", project.path(), Some("rs"))?;
    let tools_search = tools.search("TODO", project.path(), SearchOptions {
        file_type: Some("rs".to_string()),
        ..Default::default()
    })?;
    ...
    if unified_search.contains("TODO") {
        assert!(tools_search.contains("TODO") || !tools_search.is_empty(), 
...
fn test_context_persistence_across_sessions() -> Result<()> {
    ...
    // Session 2: Load and verify context
    {
        ...
        assert!(!matches!(context, WorkContext::Exploring { depth: 3, .. }), 
        ...
        assert!(!suggestions.is_empty(), "Should provide suggestions from loaded context");
    }
...
fn test_error_handling_integration() -> Result<()> {
    ...
    // StUnified errors
    assert!(unified.read(nonexistent, None, None).is_err(), 
...
fn test_realistic_multi_hour_session() -> Result<()> {
    ...
    assert!(context_size > 100, "Should have accumulated substantial context");
