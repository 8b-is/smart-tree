...
use st::st_context_aware::{
    StContextTracker, WorkContext, ContextualOperation, ProjectKnowledge, ContextualStCommand
};
use anyhow::Result;
use serde_json;
...
fn create_test_project() -> Result<TempDir> {
    ...
    fs::write(temp_dir.path().join("src/core/engine.rs"), "pub struct Engine {}")?;
    fs::write(temp_dir.path().join("tests/unit/test_engine.rs"), "#[test] fn test() {}")?;
...
fn test_context_tracker_creation() {
    ...
    match context {
        WorkContext::Exploring { depth, areas_visited } => {
...
fn test_context_detection_coding() -> Result<()> {
    ...
    let context = tracker.analyze_context()?;
    match context {
        WorkContext::Coding { language, focus_file } => {
            assert_eq!(language, "rust", "Should detect Rust language");
            assert_eq!(focus_file, PathBuf::from("/src/main.rs"), 
...
fn test_suggestions_for_coding_context() -> Result<()> {
    ...
    assert!(!suggestions.is_empty(), "Should provide suggestions for coding");
    assert!(suggestions.iter().any(|s| s.contains("relations") || s.contains("test")), 
...
fn test_suggestions_for_debugging_context() -> Result<()> {
    ...
    assert!(!suggestions.is_empty(), "Should provide debugging suggestions");
...
fn test_optimal_args_for_contexts() -> Result<()> {
    ...
    assert!(args.contains(&"--depth".to_string()), "Should include depth arg");
    ...
    let args = tracker.get_optimal_args("st");
    assert!(args.contains(&"ai".to_string()), "Coding context should use AI mode");
...
fn test_project_knowledge_tracking() -> Result<()> {
    ...
    assert!(!suggestions.is_empty(), "Should provide suggestions based on knowledge");
...
fn test_language_detection() -> Result<()> {
    ...
    for (filename, expected_lang) in test_cases {
        ...
        if let WorkContext::Coding { language, .. } = context {
            assert_eq!(language, expected_lang, 
                      "Should detect {} for {}", expected_lang, filename);
        }
    }
...
fn test_empty_project_knowledge() {
    ...
    assert!(knowledge.key_files.is_empty(), "Should start with no key files");
    assert!(knowledge.common_searches.is_empty(), "Should start with no searches");
    assert!(knowledge.hot_directories.is_empty(), "Should start with no hot directories");
    assert!(knowledge.build_commands.is_empty(), "Should start with no build commands");
    assert!(knowledge.test_patterns.is_empty(), "Should start with no test patterns");
    assert!(knowledge.doc_locations.is_empty(), "Should start with no doc locations");
}
...
fn test_work_context_serialization() -> Result<()> {
    ...
    for context in contexts {
        ...
        assert_eq!(context, deserialized, "Context should survive serialization");
    }
...
fn test_load_from_nonexistent_file() -> Result<()> {
    ...
    assert!(result.is_ok(), "Should handle missing context file gracefully");
...
fn test_all_work_context_types() -> Result<()> {
    ...
    for context in all_contexts {
        ...
        assert!(!suggestions.is_empty(), 
                "Should provide suggestions for context: {:?}", context);
    }
