...
fn main() -> Result<()> {
    ...
    for (resolution, name) in resolutions {
        ...
        println!(
            "\nMomentum: {:.3} ({})",
            patterns.momentum,
            if patterns.momentum > 0.1 {
                "📈 Increasing"
            } else if patterns.momentum < -0.1 {
                "📉 Decreasing"
            } else {
                "➡️ Stable"
            }
        ...
        println!(
            "Total Duration: {} days",
            patterns.total_duration.num_days()
        );
    ...
    for (i, context) in gatherer.contexts().iter().take(5).enumerate() {
        ...
        println!(
            "  {} - score: {:.3} (was {:.3})",
            context.ai_tool, context.relevance_score, original_score
        );
    }
    ...
    println!(
        "Found {} resonance peaks (high activity convergence)",
        resonance_peaks.len()
    );
    for (i, peak) in resonance_peaks.iter().take(5).enumerate() {
        println!("  {}. {}", i + 1, peak.format("%Y-%m-%d"));
    }
