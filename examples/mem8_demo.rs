...
use st::mem8::integration::{
    ContentType, DirectoryEvent, DirectoryHealth, DirectoryMetadata, SmartTreeMem8,
};
use std::path::Path;
...
fn main() -> Result<()> {
    ...
    for (i, memory) in src_memories.iter().take(3).enumerate() {
        println!(
            "    Memory {}: frequency={:.1}Hz, amplitude={:.2}, relevance={:.2}",
            i + 1,
    ...
    println!(
        "  Exported {} bytes (compressed from ~4.3GB grid)",
        m8_buffer.len()
    );
