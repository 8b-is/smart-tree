...
fn main() -> Result<()> {
    ...
    println!("Gathering context for project: {}\n", project_path.display());
    ...
    for (i, context) in contexts.iter().take(5).enumerate() {
        ...
        println!("   Size: {} bytes", 
            context.metadata.get("size").unwrap_or(&"unknown".to_string())
        );
        println!();
    }
    ...
    println!("M8 size: {} bytes (compressed wave-based format)", m8_data.len());
