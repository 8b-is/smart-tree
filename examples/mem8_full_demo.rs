...
fn main() {
    ...
    println!(
        "   ✓ Grid initialized: {} total cells",
        256u64 * 256u64 * 65536u64
    );
...
fn create_test_memories(wave_grid: &Arc<RwLock<WaveGrid>>) {
    let memories = [
        ...
        (
            FrequencyBand::Abstract.frequency(0.5),
            0.6,
            0.7,
            0.5,
            "Creative insight",
...
fn demonstrate_safety_features(safety_system: &SafetySystem) {
    ...
    let safe = safety_system
        .collective_intelligence
        .is_psychologically_safe();
    println!(
        "      Psychological safety: {}",
        if safe { "Maintained" } else { "At risk" }
    );
}
...
fn test_collective_emotions(cei: &Arc<CollectiveEmotionalIntelligence>) {
    ...
    println!(
        "   Psychological safety: {:.2} (threshold: 0.78)",
        collective.psychological_safety
    );
}
...
fn demonstrate_m8_format(wave_grid: &Arc<RwLock<WaveGrid>>) {
    ...
    println!(
        ...
        text.len() as f64 / buffer.len() as f64
    );
...
fn run_consciousness_simulation(
    ...
    while start.elapsed() < Duration::from_secs(5) {
        ...
        if !state.active_memories.is_empty() {
            ...
            if cycle_count % 10 == 0 {
                println!(
                    "   Cycle {}: {} active memories, awareness={:.2}, safety={:?}",
                    cycle_count,
