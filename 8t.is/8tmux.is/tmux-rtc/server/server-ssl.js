...
try {
  ...
  const privateKey = fs.readFileSync(process.env.SSL_KEY || '/path/to/privkey.pem', 'utf8');
  const certificate = fs.readFileSync(process.env.SSL_CERT || '/path/to/cert.pem', 'utf8');
  const ca = process.env.SSL_CA ? fs.readFileSync(process.env.SSL_CA, 'utf8') : undefined;
  ...
  console.log(`🔒 SSL enabled! TMUX-RTC Server surfing securely on port ${PORT}! 🚢`);
} catch (error) {
  console.log('⚠️ SSL certificates not found, falling back to HTTP');
  // Fallback to HTTP
  server = app.listen(HTTP_PORT, () => {
    console.log(`🌊 TMUX-RTC Server surfing on HTTP port ${HTTP_PORT}! Aye, Aye! 🚢`);
  });
}

// If SSL is enabled, also start HTTP redirect server
if (server !== app) {
